...
namespace Microsoft.Xna.Framework.Graphics
{
	internal class ESTexture2D : IDisposable
	{
		...
		private ALL20 _originalFilter;
		private ALL20 _originalWrap;
		...
		{
			Console.WriteLine("RetryToCreateTexture");
		}
		...
		public ESTexture2D (IntPtr data, SurfaceFormat pixelFormat, int width, int height, Size size, All filter)
		{
			...
			InitWithData (data, pixelFormat, width, height, size, filter, ALL20.Repeat);
		}
		...
		public void InitWithBitmap(Bitmap imageSource, All filter)
			...
			InitWithBitmap(imageSource, filter, All.Repeat);
		}

		private void InitWithBitmap (Bitmap image, All filter, All wrap)
		{
			...
			if (!IsPowerOfTwo (_width) || !IsPowerOfTwo (_height)) {
				filter = ALL20.Linear;
				wrap = ALL20.ClampToEdge;
			}
			// when we load a image which is not xnb, the color order is not what the opengl expects
			InitWithData (bitmapData.Scan0, _format, image.Width, image.Height, new Size (image.Width, image.Height), filter, ALL20.Repeat);
		...
		{	
			...
			GL.BindTexture (TextureTarget.Texture2D, (int)_name);
			...
			case SurfaceFormat.Dxt3:
				...
				GL.TexImage2D (TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, (int)width, (int)height, 0, GLPixelFormat.Rgba, PixelType.UnsignedByte, data);
					//GL.TexImage2D (TextureTarget.Texture2D, 0, (int)All.Rgba, (int)width, (int)height, 0, All.Rgba, All.UnsignedByte, data);
				break;
			case SurfaceFormat.Bgra4444 /*kTexture2DPixelFormat_RGBA4444*/:
				GL.TexImage2D (TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, (int)width, (int)height, 0, GLPixelFormat.Rgba, PixelType.UnsignedShort4444, data);
					...
					//GL.TexImage2D (TextureTarget.Texture2D, 0, (int)All.Rgba, (int)width, (int)height, 0, All.Rgba, All.UnsignedShort4444, data);
				break;
			case SurfaceFormat.Bgra5551 /*kTexture2DPixelFormat_RGB5A1*/:
				GL.TexImage2D (TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba, (int)width, (int)height, 0, GLPixelFormat.Rgba, PixelType.UnsignedShort5551, data);
					...
					//GL.TexImage2D (TextureTarget.Texture2D, 0, (int)All.Rgba, (int)width, (int)height, 0, All.Rgba, All.UnsignedShort5551, data);
				break;
			case SurfaceFormat.Alpha8 /*kTexture2DPixelFormat_A8*/:
				GL.TexImage2D (TextureTarget.Texture2D, 0, PixelInternalFormat.Alpha, (int)width, (int)height, 0, GLPixelFormat.Alpha, PixelType.UnsignedByte, data);
					...
					//GL.TexImage2D (TextureTarget.Texture2D, 0, (int)All.Alpha, (int)width, (int)height, 0, All.Alpha, All.UnsignedByte, data);
				break;
			default:
...
//		}
		...
		{
            		Console.WriteLine("ForceRetryToCreateTexture");
		}
